...
def github_pull_request_merge(parser, xml_parent, data):
    ...
    :arg str only-admins-merge: if `true` only administrators can merge the
        pull request, (default false)
    :arg str disallow-own-code: if `true` will allow merging your own pull
        requests, in opposite to needing someone else to trigger the merge.
        (defaul false)
    :arg str merge-comment: Comment to set on the merge commit (default None)
    :arg str fail-on-non-merge: fail the job if the merge was unsuccessful
        (defaul false)
    :arg str delete-on-merge: Delete the branch of the pull request on
